...
from pb.addressbook.models.addressbook_pb2 import AddressBook as CppAddressBook  # noqa: E402
from pb.people.models.people_pb2 import Person as CppPerson  # noqa: E402
from pyrobuf_flat_pb import AddressBook as PyroAddressBook  # noqa: E402
...
def benchmark_memory(name, baseline_proto, proto_class, baseline_allocated_memory=None):
    ...
    if baseline_allocated_memory is not None:
        percentage_drop = (
            (baseline_allocated_memory - allocated_memory) / baseline_allocated_memory
        ) * 100
        drop_label = "Decrease"
...
def main():
    ...
    for item_count in items:
        ...
        longest_implementation = len(
            max(
                chain.from_iterable(
                    [[keys for keys in variant.keys()] for variant in benchmarks.values()]
                ),
                key=len,
            )
        )
        for title, variants in benchmarks.items():
            ...
            for implementation, function in variants.items():
                result = run_timeit(
                    function, f"\t\t{implementation.ljust(longest_implementation)}", baseline
                )
